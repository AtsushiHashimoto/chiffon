    ...
    $(document)
        .on('webkitfullscreenchange', function (e) {
            ...
            if (DEBUG) console.log({
                fullscreenstate: $(':-webkit-full-screen')
                    .length ? true : false
            });
            var value = $(':-webkit-full-screen')
                .length ? 'ON' : 'OFF';
        ...
        if (data.status == 'success') {
            ...
            $.each(data.body, function (i, obj) {
                ...
                if (obj.ChannelSwitch) {
                    // ChannelSwitch
                    if (DEBUG) console.log({
                        '-- ChannelSwitch': obj.ChannelSwitch
                    });
        ...
        .each(function () {
            ...
            $(media)
                .on('seeked', function (e) {
                    ...
                    seeked_timer = setTimeout(time_change, 1000, {
                        pk: id,
                        v: media.currentTime
                    });
                });
            $(media)
                .on('volumechange', function (e) {
                    ...
                    if (mute_status != media.muted) {
                        ...
                        volume_timer = setTimeout(volume_change, 1000, {
                            pk: id,
                            v: media.volume
                        });
                    }
